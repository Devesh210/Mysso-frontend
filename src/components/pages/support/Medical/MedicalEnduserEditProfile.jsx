...
const MedicalEnduserEditProfile = () => {
    ...
    const getEnduserdata = async (id) => {
        try {
            ...
            await fetch(`${API_URL}/api/getEnduserById?id=${id}`, requestOption)
                ...
                .then((data) => {
                    console.log(data)
                    const enduser = data?.data[0]
                    setEnduserdata(data?.data[0])
                    setId(enduser?._id)
                    setFullName(enduser?.fullName)
                    setEmail(enduser?.email)
                    setContactNumber(enduser?.contactNumber)
                    setProfilePicture(enduser?.profilePicture[0])
                    setAge(enduser?.age)
                    setGender(enduser?.gender)
                    setCountry(enduser?.country)
                    setState(enduser?.state)
                    setCity(enduser?.city)
                    setPreferredLanguage(enduser?.preferredLanguage)
                    setEmergencyContactName(enduser?.emergencyContactName)
                    setRelationship(enduser?.relationship)
                    setEmergencyCountry(enduser?.emergencyCountry)
                    setEmergencyState(enduser?.emergencyState)
                    setEmergencyCity(enduser?.emergencyCity)
                    setEmergencyContactNumber(enduser?.emergencyContactNumber)
                    setPastTreatments(enduser?.pastTreatments)
                    setRegularMedication(enduser?.regularMedication)
                    setInsuranceProvider(enduser?.insuranceProvider)
                    setCoverageDetails(enduser?.coverageDetails)
                })
        } catch (error) {
            console.log(error)
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!fullName) {
                ...
                await fetch(`${API_URL}/api/updateEndUserData`, requestoptions)
                    .then(response => response.json())
                    .then(data => {
                        console.log(data);
                        if (data.status == 200) {
                        ...
                        <Row>
                            ...
                            <Col lg={4} className='mb-2'>
                                ...
                                <Form.Group className="mb-3" controlId="formEmergencyCountry">
                                    <Form.Select className='mb-3' value={emergencyCountry} onChange={handleEmergencyCountrychange} ref={emergencyCountryRef}>
                                        <option hidden>Select Country </option>
                                        {countryList?.map((country, index) => (
                                            <option key={index} value={country._id}>{country.country_name}</option>
                                        ))}
                            ...
                            <Col lg={4} className='mb-2'>
                                ...
                                <Form.Group className="mb-3" controlId="formEmergencyState">
                                    <Form.Select className='mb-3' value={emergencyState} onChange={handleEmergencyStatechange} ref={emergencyStateRef}>
                                        <option hidden>Select State</option>
                            ...
                            <Col lg={4} className='mb-2'>
                                ...
                                <Form.Group className="mb-3" controlId="formEmergencyCity">
                                    <Form.Select className='mb-3' value={emergencyCity} onChange={handleEmergencyCitychange} ref={emergencyCityRef}>
                                        <option hidden>Select City</option>
...
export default MedicalEnduserEditProfile;