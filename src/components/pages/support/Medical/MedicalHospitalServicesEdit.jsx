...
const MedicalHospitalServicesEdit = () => {
            ...
            await fetch(`${API_URL}/api/gethospitalServicesbyId?hospital_id=${id}`, requestOptions)
                ...
                .then(data => {
                    if (data.status === 200) {
                        ...
                        setServiceCategory(hospitaldata?.serviceCategory)
                        setServiceName(hospitaldata?.serviceName)
                        setServiceDescription(hospitaldata?.serviceDescription)
                        setStartDate(hospitaldata?.startDate)
                        setEndDate(hospitaldata?.endDate)
                        setStartTime(hospitaldata?.startTime)
                        setEndTime(hospitaldata?.endTime)
                        setServiceLocation(hospitaldata?.serviceLocation)
                        setServiceContact(hospitaldata?.serviceContact)
                        setDoctorName(hospitaldata?.doctorName)
                        setDoctorSpecialization(hospitaldata?.doctorSpecialization)
                        setYearsOfExperience(hospitaldata?.yearsOfExperience)
                    } else {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!serviceCategory) {
                ...
                await fetch(`${API_URL}/api/updatehospitalServicesbyId`, requestOption)
                    .then((res) => res.json())
                    .then((data) => {
                        console.log(data)
                        if (data.status === 200) {
                            ...
                            Navigate(`/MedicalHospitalServicesView?id=${services?._id}`);
                        } else {
                            swal({
                                text: data.message,
                                icon: "error",
...
export default MedicalHospitalServicesEdit;