...
const EducationExpertEditprofile = () => {
    ...
    const getExpertData = async (id) => {
        try {
            ...
            await fetch(`${API_URL}/api/getEducationExpertById?id=${id}`, requestOption)
                ...
                .then((data) => {
                    console.log(data)
                    const expertdetails = data?.data[0];
                    setExpertData(expertdetails);
                    setId(expertdetails?._id);
                    setProfile_pic(expertdetails?.profile_pic[0]);
                    setBusinesscardfront(expertdetails?.businesscardfront ? expertdetails?.businesscardfront[0] : []);
                    setBusinesscardback(expertdetails?.businesscardback ? expertdetails?.businesscardfront[0] : []);
                    setFullName(expertdetails?.fullName);
                    setEmail(expertdetails?.email);
                    setContact(expertdetails?.contact);
                    setCountry(expertdetails?.country);
                    setState(expertdetails?.state);
                    setCity(expertdetails?.city);
                    setBio(expertdetails?.bio);
                    setOccupation(expertdetails?.occupation);
                    setExperience(expertdetails?.experience);
                    setIndustry(expertdetails?.industry);
                    setEducation(expertdetails?.education);
                    setCertifications(expertdetails?.certifications);
                    setWorkExperience(expertdetails?.work_experience);
                    const careerFieldsdata = expertdetails?.career_fieldsdetails?.map(val => ({
                        value: val?._id,
                        label: val?.education_fieldofstudy
                    }));
                    setCareerFields(careerFieldsdata);
                    const guidanceTypedata = expertdetails?.guidance_typedetails?.map(val => ({
                        value: val?._id,
                        label: val?.education_typeof_guidance
                    }));
                    setGuidanceType(guidanceTypedata);
                    const targetAudiencedata = expertdetails?.target_audiencedetails?.map(val => ({
                        value: val?._id,
                        label: val?.education_target_audience
                    }));
                    setTargetAudience(targetAudiencedata);
                    setSkills(expertdetails?.skills);
                    const consultationModedata = expertdetails?.consultation_modedetails?.map(val => ({
                        value: val?._id,
                        label: val?.education_consultation
                    }));
                    setConsultationMode(consultationModedata);
                    setTimeSlots(expertdetails?.time_slots);
                    setConsultationLanguage(expertdetails?.consultation_language);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (!fullName) {
                ...
                await fetch(`${API_URL}/api/updateEducationExpertData`, requestoptions)
                    .then(response => response.json())
                    .then(data => {
                        console.log(data);
                        if (data.status === 200) {
                                ...
                                <Form.Group className="mb-3" controlId="formCountry">
                                    <Form.Select className='mb-3' onChange={handleCountrychange} ref={countryRef}>
                                        ...
                                        {countryList?.map((val, index) => (
                                            <option key={index} selected={country == val._id} value={val._id}>{val.country_name}</option>
                                        ))}
                                    </Form.Select>
                                </Form.Group>
                            </Col>
                                ...
                                <Form.Group className="mb-3" controlId="formState">
                                    <Form.Select className='mb-3' onChange={handleStatechange} ref={stateRef}>
                                        ...
                                        {stateList?.map((val, index) => (
                                            <option key={index} selected={state == val._id} value={val._id}>{val.state_name}</option>
                                        ))}
                                    </Form.Select>
                                </Form.Group>
                            </Col>
                                ...
                                <Form.Group className="mb-3" controlId="formCity">
                                    <Form.Select className='mb-3' onChange={handleCitychange} ref={cityRef}>
                                        ...
                                        {cityList?.map((val, index) => (
                                            <option key={index} selected={city == val._id} value={val._id}>{val.city_name}</option>
                                        ))}
                                    </Form.Select>
                                </Form.Group>
                            </Col>
                                ...
                                <Row>
                                    <Col lg={4} className='mb-2'>
                                        ...
                                        <div className='profileimgc' >
                                            ...
                                            <img src={image != profile ? image : `${API_URL}/uploads/company_logo/${profile_pic?.filename}`} alt="Profile" />
                        ...
                        <Row>
                            <Col lg={4} className='mb-2'>
                                ...
                                <Form.Group className="mb-3" controlId="formCareerFields">
                                    {/* <Form.Select className="mb-3" onChange={(e) => setCareerFields(e.target.value)}>
                                        <option value="" hidden>Select career field</option>
                                        {careerFieldsList?.map((val, index) => (
                                            <option key={index} selected={careerFields == val?._id} value={val?._id}>{val?.education_fieldofstudy}</option>
                                        ))}
                                    </Form.Select> */}
                                    <Select
                            ...
                            <Col lg={4} className='mb-2'>
                                ...
                                <Form.Group className="mb-3" controlId="formGuidanceType">
                                    {/* <Form.Select className="mb-3"  onChange={(e) => setGuidanceType(e.target.value)} ref={guidanceTypeRef}>
                                        <option value="" hidden>Select guidance type</option>
                                        {guidanceTypeList?.map((val, index) => (
                                            <option key={index} selected={guidanceType == val?._id} value={val._id}>{val.education_typeof_guidance}</option>
                                        ))}
                                    </Form.Select> */}
                                    <Select
                                        closeMenuOnSelect={false}
                            ...
                            <Col lg={4} className='mb-2'>
                                ...
                                <Form.Group className="mb-3" controlId="formTargetAudience">
                                    {/* <Form.Select className="mb-3" onChange={(e) => setTargetAudience(e.target.value)} ref={targetAudienceRef}>
                                        <option value="" hidden>Select target audience</option>
                                        {targetAudienceList?.map((val, index) => (
                                            <option key={index} selected={targetAudience == val?._id}  value={val._id}>{val.education_target_audience}</option>
                                        ))}
                                    </Form.Select> */}
                                    <Select
                                        closeMenuOnSelect={false}
                        ...
                        <Row>
                            <Col lg={4} className='mb-2'>
                                ...
                                <Form.Group className="mb-3" controlId="formConsultationMode">
                                    {/* <Form.Select className="mb-3"  onChange={(e) => setConsultationMode(e.target.value)} ref={consultationModeRef}>
                                        <option value="" hidden>Select consultation mode</option>
                                        {consultationModeList?.map((val, index) => (
                                            <option key={index} selected={consultationMode == val?._id} value={val._id}>{val.education_consultation}</option>
                                        ))}
                                    </Form.Select> */}
                                    <Select
                                        closeMenuOnSelect={false}
...
export default EducationExpertEditprofile;
