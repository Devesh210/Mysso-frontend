...
const EducationStudentEditprofile = () => {
    ...
    const getStudentData = async (id) => {
        try {
            ...
            await fetch(`${API_URL}/api/getEducationStudentById?id=${id}`, requestOption)
                ...
                .then((data) => {
                    console.log(data)
                    const studentdetails = data?.data[0];
                    setStudentData(studentdetails)
                    setId(studentdetails?._id);
                    setProfile_pic(studentdetails?.profile_pic[0])
                    setFullName(studentdetails?.fullName)
                    setEmail(studentdetails?.email)
                    setContact(studentdetails?.contact)
                    setBio(studentdetails?.bio)
                    setCurrentEducation(studentdetails?.currentEducation)
                    setFieldStudy(studentdetails?.fieldStudy)
                    setSchoolCollege(studentdetails?.schoolCollege)
                    setQualification(studentdetails?.qualification)
                    setCareerInterest(studentdetails?.careerInterest)
                    setCareerPath(studentdetails?.careerPath)
                    setSkills(studentdetails?.skills)
                    setGoals(studentdetails?.goals)
                    setAge(studentdetails?.age)
                    setGender(studentdetails?.gender)
                    setCountry(studentdetails?.country)
                    setState(studentdetails?.state)
                    setCity(studentdetails?.city)
                    setLanguages(studentdetails?.languages)
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (!fullName) {
                ...
                await fetch(`${API_URL}/api/updateEducationStudentData`, requestoptions)
                    .then(response => response.json())
                    .then(data => {
                        console.log(data);
                        if (data.status == 200) {
...
console.log(profile_pic?.filename)
console.log(image)
                        ...
                        <Row>
                            ...
                            <Col lg={4} className='mb-2'>
                                ...
                                <div className='profileimgc'>
                                    ...
                                    {/* <img src={`${API_URL}/uploads/user_profile/${profile_pic.filename}`} alt="Profile" /> */}

                                    <img src={image != profile ? image : `${API_URL}/uploads/user_profile/${profile_pic.filename}`} alt="Profile" />
                        ...
                        <Row>
                            <Col lg={4} className='mb-2'>
                                ...
                                <Form.Group className="mb-3" controlId="formCurrentEducation">
                                    <Form.Select className='mb-3'  onChange={(e) => setCurrentEducation(e.target.value)} ref={currentEducationRef}>
                                        <option hidden>Select Current Education</option>
                                        {educationList.map((education, index) => (
                                            <option key={index} selected={currentEducation == education?._id}  value={education?._id}>{education?.education_list}</option>
                                        ))}
                                    </Form.Select>
                                </Form.Group>
                            </Col>
                                ...
                                <Form.Group className="mb-3" controlId="formFieldStudy">
                                    <Form.Select className='mb-3' value={fieldStudy} onChange={(e) => setFieldStudy(e.target.value)} ref={fieldStudyRef}>
                                        ...
                                        {fieldofstudyList.map((fieldofstudy, index) => (
                                            <option key={index} selected={fieldStudy == fieldofstudy?._id} value={fieldofstudy?._id}>{fieldofstudy?.education_fieldofstudy}</option>
                                        ))}
                                    </Form.Select>
                                </Form.Group>
                            </Col>
                            ...
                            <Col lg={4} className='mb-2'>
                                ...
                                <Form.Group className="mb-3" controlId="formQualification">
                                    <Form.Select className='mb-3' onChange={(e) => setQualification(e.target.value)}>
                                        <option hidden>Select Qualification</option>
                                        {educationList.map((education, index) => (
                                            <option key={index} selected={qualification == education?._id} value={education?._id}>{education?.education_list}</option>
                                        ))}
                                    </Form.Select>
                                </Form.Group>
                            </Col>
...
export default EducationStudentEditprofile;
