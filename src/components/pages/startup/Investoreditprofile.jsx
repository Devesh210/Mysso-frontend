...
const Investoreditprofile = () => {
        ...
        try {
            ...
            await fetch(`${API_URL}/api/getInvestorId?id=${id}`, requestOption)
                ...
                .then((data) => {
                    ...
                    setEmailId(investor?.emailid)
                    setPhoneNumber(investor?.phoneNumber)
                    setLinkedIn(investor?.linkedIn)
                    setCountry_id(investor?.country_id)
                    setState_id(investor?.state_id)
                    setCity_id(investor?.city_id)
                    setTimezone(investor?.timezone)

                    setCurrentRole(investor?.currentRole)
                    setCompanyName(investor?.companyName)
                    setInvestorIndustry(investor?.investorIndustry)
                    setYearOfExperience(investor?.yearofexperience)
                    setProfessionalBio(investor?.professionalbio)
                    setPreviousInvestments(investor?.previousInvestments)

                    const investmentstagedata = investor?.preferredInvestmentStage_details?.map((val) => ({
                        value: val._id,
                        label: val.preferred_investment_stage,
                    }));
                    setPreferredInvestmentStage(investmentstagedata)
                    const industrysectordata = investor?.IndustrySector_details.map(industry => ({
                        value: industry._id,
                        label: industry.startup_industry,
                    }));
                    setPreferredIndustrySectors(industrysectordata )
                    setPreferredGeographic(investor?.preferredGeographic)
                    setMinimumInvestmentAmount(investor?.minimumInvestmentAmount)
                    setMaximumInvestmentAmount(investor?.maximumInvestmentAmount)
                    const investmentTypedata = investor?.InvestmentType_details.map(investmentType => ({
                        value: investmentType._id,
                        label: investmentType.startup_investment,
                    }));
                    setPreferredInvestmentType(investmentTypedata)

                    const startupStagedata = investor?.StartupStage_details.map(stage => ({
                        value: stage._id,
                        label: stage.startup_stage,
                    }));
                    setPreferredStartupStage(startupStagedata)
                    const revenueModeldata = investor?.revenuemodel_details.map(model => ({
                        value: model._id,
                        label: model.startup_revenue,
                    }));
                    setRevenuemodel(revenueModeldata)

                    setStartupsInvested(investor?.startupsInvested)
                    setNotableInvestments(investor?.notableInvestments)
                    setAverageHoldingPeriod(investor?.averageHoldingPeriod)
                    setExitStrategiesPreferred(investor?.exitStrategiesPreferred)
                    setSuccessStories(investor?.successStories)
                })
        } catch (error) {
            console.log(error)
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await fetch(`${API_URL}/api/updateinvestordetail`, requestoptions)
                .then(response => response.json())
                .then(data => {
                    console.log(data);
                    if (data.status == 200) {
                        ...
                        <Row>
                            <Col lg={4} className='mb-2'>
                                ...
                                <Form.Group className="mb-3" controlId="formBasicEmail">
                                    <Form.Control type="text" placeholder="Enter Full Name" defaultValue={fullName} onChange={(e) => setFullName(e.target.value)} ref={fullNameRef} />
                                </Form.Group>
                            </Col>
                            <Col lg={4} className='mb-2'>
                                <label>Email Address <span className='labelerrorssss'>*</span></label>
                                <Form.Group className="mb-3" controlId="formBasicEmail">
                                    <Form.Control type="text" placeholder="Enter Email Address" defaultValue={emailid} onChange={(e) => setEmailId(e.target.value)} ref={emailRef} />
                                </Form.Group>
                            </Col>
                            <Col lg={4} className='mb-2'>
                            ...
                            <Col lg={4} className='mb-2'>
                                ...
                                <Form.Group className="mb-3" controlId="formBasicEmail">
                                    <Form.Control type="text" placeholder="Enter LinkedIn Profile" defaultValue={linkedIn} onChange={(e) => setLinkedIn(e.target.value)} />
                                </Form.Group>
                            </Col>
                            <Col lg={4} className='mb-2'>
                            ...
                            <Col lg={4} className='mb-2'>
                                ...
                                <Form.Group className="mb-3" controlId="timezone">
                                    <Form.Select className='mb-3' value={timezone} onChange={(e) => setTimezone(e.target.value)} ref={timezoneRef}>
                    ...
                    <div className='descr-content'>
                        <Row>
                            <Col lg={4} className='mb-2'>
                                ...
                                <Form.Group className="mb-3" controlId="formBasicEmail">
                                    <Form.Control type="text" placeholder="Enter Current Role/Title" defaultValue={currentRole} onChange={(e) => setCurrentRole(e.target.value)} ref={currentRoleRef} />
                                </Form.Group>
                            </Col>
                            <Col lg={4} className='mb-2'>
                                ...
                                <Form.Group className="mb-3" controlId="formBasicEmail">
                                    <Form.Select className='mb-3' onChange={(e) => setInvestorIndustry(e.target.value)}>
                                        ...
                                        {industryList?.map((val, index) => (
                                            <option key={index} selected={investorIndustry == val._id} value={val._id}>{val.startup_industry}</option>
                                        ))}
                                    </Form.Select>
                                </Form.Group>
                            </Col>
                            <Col lg={4} className='mb-2'>
                                ...
                                <Form.Group className="mb-3" controlId="formBasicEmail">
                                    <Form.Control type="text" placeholder="Enter Years of Experience" onInput={allowOnlyNumbers} defaultValue={yearofexperience} onChange={(e) => setYearOfExperience(e.target.value)} ref={yearOfExperienceRef} />
                                </Form.Group>
                            </Col>
                            <Col lg={4} className='mb-2'>
                            ...
                            <Col lg={4} className='mb-2'>
                                ...
                                <Form.Group className="mb-3" controlId="formBasicEmail">
                                    <Form.Control
                                        ...
                                        defaultValue={previousInvestments}
                    ...
                    <div className='descr-content'>
                        <Row>
                            ...
                            <Col lg={4} className='mb-2'>
                                ...
                                <Form.Group className="mb-3" controlId="formBasicEmail">
                                    <Form.Select className='mb-3' value={preferredGeographic} onChange={(e) => setPreferredGeographic(e.target.value)} ref={preferredGeographicRef}>
                            ...
                            <Col lg={4} className='mb-2'>
                                ...
                                <Form.Group className="mb-3" controlId="formBasicEmail"  >
                                    <Form.Control type="text" placeholder="Enter Minimum Investment Amount" onInput={allowOnlyNumbers} defaultValue={minimumInvestmentAmount} onChange={(e) => setMinimumInvestmentAmount(e.target.value)} ref={minimumInvestmentAmountRef} />
                                </Form.Group>
                            </Col>
                            <Col lg={4} className='mb-2'>
                                <label>Maximum Investment Amount <span className='labelerrorssss'>*</span></label>
                                <Form.Group className="mb-3" controlId="formBasicEmail">
                                    <Form.Control type="text" placeholder="Enter Maximum Investment Amount" onInput={allowOnlyNumbers} defaultValue={maximumInvestmentAmount} onChange={(e) => setMaximumInvestmentAmount(e.target.value)} ref={maximumInvestmentAmountRef} />
                                </Form.Group>
                            </Col>
                            <Col lg={4} className='mb-2'>
                    ...
                    <div className='descr-content'>
                        <Row>
                            <Col lg={4} className='mb-2'>
                                ...
                                <Form.Group className="mb-3" controlId="formBasicEmail">
                                    <Form.Control type="text" placeholder="Enter Number of Startups Invested In" onInput={allowOnlyNumbers} defaultValue={startupsInvested} onChange={(e) => setStartupsInvested(e.target.value)} ref={startupsInvestedRef} />
                                </Form.Group>
                            </Col>
                            <Col lg={4} className='mb-2'>
                                ...
                                <Form.Group className="mb-3" controlId="formBasicEmail">
                                    <Form.Control
                                        ...
                                        defaultValue={notableInvestments}
                            ...
                            <Col lg={4} className='mb-2'>
                                ...
                                <Form.Group className="mb-3" controlId="formBasicEmail">
                                    <Form.Control type="text" placeholder="Enter Average Holding Period" defaultValue={averageHoldingPeriod} onChange={(e) => setAverageHoldingPeriod(e.target.value)} ref={averageHoldingPeriodRef} />
                                </Form.Group>
                            </Col>
                            <Col lg={4} className='mb-2'>
                                ...
                                <Form.Group className="mb-3" controlId="formBasicEmail">
                                    <Form.Select className='mb-3' onChange={(e) => setExitStrategiesPreferred(e.target.value)} ref={exitStrategiesPreferredRef}>
                                        ...
                                        {exitstrategyList?.map((val, index) => (
                                            <option key={index} selected={exitStrategiesPreferred == val._id} value={val._id}>{val.exit_strategy}</option>
                                        ))}
                                    </Form.Select>
                                </Form.Group>
                            </Col>
